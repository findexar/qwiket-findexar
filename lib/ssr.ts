...
import fetchMetaLink, {
  promiseGetPlayerPhoto,
} from "@lib/server-actions/meta-link";
...
import fetchTeamPlayers, {
  actionGetTeamName,
} from "@lib/server-actions/team-players";
...
import fetchChat, {
  actionGetPromptPage,
  promiseGetPromptPage,
  promisePromptChatResponse,
  ssrPromptChatResponse,
} from "@lib/server-actions/chat";
...
import promiseFetchBlogArticle, {
  promiseFetchBlogArticles,
...
export const ssrPrepParams = async (
  params: SSRParams,
  ...
  let bot = botInfo.bot || ua.match(/vercel|spider|crawl|curl|ShortLinkTranslate|MetaInspector|Googlebot/i);
  if (!ua) {
    bot = true;
  }
  let userId = "";
  ...
  if (bot) {
    ...
    await actionRecordEvent(
      "human-ssr",
      `{"utm_content":"${utm_content}","params":"${JSON.stringify({
        tab,
        view,
        rtab,
        prompt,
  ...
  if (
    league &&
    !["NFL", "MLB", "NBA", "NHL", ""].includes(league.toUpperCase())
  ) {
   ...
   *
   */
  if (league) calls.push(await fetchLeagueTeams({ league }));
  ...
  if (findexarxid) {
    // if a mention story is opened
    calls.push(await fetchMention({ type: "AMention", findexarxid }));
    calls.push(await fetchMetaLink({ func: "meta", findexarxid, long: 1 }));
  }
  if (story || cstory) {
    // if a digest story is opened
    calls.push(
      await fetchSlugStory({ type: "ASlugStory", slug: story || cstory })
    );
  }
  if (m || cm) {
    calls.push(await fetchSlugStory({ type: "AMentionStory", m: m || cm }));
  }
  //  if (!story && !findexarxid && !m)
  if (teamid) calls.push(await fetchTeamPlayers({ userId, sessionid, teamid }));
  ...
  if ((!rtab && !teamid && !athleteUUId) || tab == "mentions") {
    if (!teamid && !athleteUUId)
      ...
      calls.push(
        await fetchTeamMentions({ userId, sessionid, league, teamid })
      );
    else if (teamid && athleteUUId)
      calls.push(
        await fetchPlayerMentions({
          userId,
          sessionid,
          league,
          teamid,
  ...
  try {
    if (tab == "blog") {
      if (b) {
        const blogArticleKey: BlogArticleKey = {
        ...
        calls.push(
          await promiseFetchBlogArticles({
  ...
  if (promptUUId && tab == "chat") {
    ...
    if (response) {
      ...
      articleStructuredData = {
        "@context": "https://schema.org",
        "@type": "Article",
        "@id": `${process.env.NEXT_PUBLIC_SERVER} /${leagueid}/${teamid} /${
  ...
  if (tab == "prompts") {
    const promptPageKey: PromptPageKey = {
      type: "prompt-page",
      pageUUId: null,
      search_key: `${athleteUUId ? athleteUUId : teamid ? teamid : ""}`,
    };
...
export async function generateMetadata(
  {
    params,
    searchParams,
  }: { params: SSRParams; searchParams: SSRSearchParams },
  parent: ResolvingMetadata
): Promise<Metadata> {
  // Read route params
  const { id, story, tab, view, m, cstory, cm, promptUUId, b } =
    searchParams as any;
  let { leagueid = "", teamid = "", name = "", athleteUUId = "" } = params;
  ...
  const astoryImageOgUrl = astoryOgImage
    ? astoryOgImage
    : astoryImage
    ? `${process.env.NEXT_PUBLIC_SERVER}/api/og.png/${encodeURIComponent(
        astoryImage
      )}/${encodeURIComponent(astorySite_Name)}/${image_width}/${image_height}`
    : `q-logo-white-2048.png`;
  ...
  let ogDescription =
    amentionSummary ||
    promptResponse ||
    "Interactive sports knowledge for fantasy sports and sports betting Fans";
  ...
  let ogTitle =
    ogTarget ||
    `Qwiket : Helping Fantasy Sports and Sports Betting Fans to Elevate their Game`;
  if (astory) {
    ogUrl = `${process.env.NEXT_PUBLIC_SERVER}/${leagueid}/${teamid}/${
      athleteUUId ? `${encodeURIComponent(name)}/${athleteUUId}/` : ""
  ...
  if (cstory && astory?.publishedTime) {
    const publishedDate = astory.publishedTime
      ? new Date(astory.publishedTime)
      : new Date();
  ...
  if (promptResponse && promptUUId) {
    ...
    ogUrl = `${process.env.NEXT_PUBLIC_SERVER}/${leagueid}/${teamid}/${
      athleteUUId ? `${encodeURIComponent(name)}/${athleteUUId}/` : ""
    }?tab=chat&prompt=${encodeURIComponent(
      promptResponse.prompt
    )}&promptUUId=${promptUUId}`;
    // Update noindex based on publishedTime being older than 1 week
    const publishedDate = promptResponse.publishedTime
      ? new Date(promptResponse.publishedTime)
      : new Date();
    const oneWeekAgo = new Date();
    oneWeekAgo.setDate(oneWeekAgo.getDate() - 7);
    noindex = 0;
  ...
  if (
    ...
    ogUrl = `${process.env.NEXT_PUBLIC_SERVER}/${leagueid}/${teamid}/${
      athleteUUId ? `${encodeURIComponent(name)}/${athleteUUId}/` : ""
    }${tab ? `?tab=${tab}&utm_content=${encodeURIComponent(tab)}` : ""}`;
  }
  if (tab == "blog" && b) {
    const blogArticleKey: BlogArticleKey = {
      type: "fetch-blog-article",
